...
istream& operator >> (istream& stream, Date& new_date) {
	...
	for (const auto& i : str) {
		if (i == '-') {
			num_defs_near++;			//ïðîâåðèì äåôèñû ïîäðÿä
		}
		else {
			num_defs_near = 0;
		}
		...
		if (count(allowed_symbols.begin(), allowed_symbols.end(), i) == 0) {  //ïðîâåðèì ðàçðåøåííûå ñèìâîëû
			throw exep;
		}
	}
	int num_defs = count(str.begin(), str.end(), '-');
	if ( num_defs < 2 || num_defs > 5) {					//ïðîâåðèì êîëè÷åñòâî äåôèñîâ
		throw exep;
	}
	ss >> new_year;
	ss.ignore(1);
